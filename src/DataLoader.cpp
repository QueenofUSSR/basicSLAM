    ...
    : currentIndex(0), fx_(700.0), fy_(700.0), cx_(0.5), cy_(0.5)
{
    ...
    // 使用 cv::glob 列出文件，捕获可能的 OpenCV 异常
    try {
        cv::glob(imageDir + "/*", imageFiles, false);
    } catch(const cv::Exception &e){
        std::cerr << "DataLoader: cv::glob failed for '" << imageDir << "': " << e.what() << std::endl;
        imageFiles.clear();
        return;
    }
...
bool DataLoader::getNextImage(cv::Mat &image, std::string &imagePath){
