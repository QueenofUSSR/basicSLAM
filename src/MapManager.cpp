...
std::vector<int> MapManager::findVisibleCandidates(const cv::Mat &lastR, const cv::Mat &lastT,
    ...
    for(size_t mi=0; mi<mappoints_.size(); ++mi){
        ...
        cv::Mat Xw = (cv::Mat_<double>(3,1) << mp.p.x, mp.p.y, mp.p.z);
        cv::Mat Xc = lastR.t() * (Xw - lastT);
...
std::vector<MapPoint> MapManager::triangulateBetweenLastTwo(const std::vector<cv::Point2f> &pts1n,
    ...
    cv::Mat R_last = lastKf.R_w, t_last = lastKf.t_w;
    cv::Mat R_cur = curKf.R_w, t_cur = curKf.t_w;
    cv::Mat R_rel = R_last.t() * R_cur;
    cv::Mat t_rel = R_last.t() * (t_cur - t_last);
    ...
    cv::Mat points4D;
    try{ cv::triangulatePoints(P1, P2, pts1n, pts2n, points4D); }
    ...
    for(int c=0;c<p4d64.cols; ++c){
        ...
        if(Xz <= 0) continue;
        cv::Mat Xc = (cv::Mat_<double>(3,1) << Xx, Xy, Xz);
        cv::Mat Xw = lastKf.R_w * Xc + lastKf.t_w;
        MapPoint mp; mp.p = cv::Point3d(Xw.at<double>(0,0), Xw.at<double>(1,0), Xw.at<double>(2,0));
        ...
        cv::Mat ray1 = Xc_last / cv::norm(Xc_last);
        cv::Mat ray2 = Xc_cur / cv::norm(Xc_cur);
...
void MapManager::applyOptimizedKeyframePose(int keyframeId, const cv::Mat &R, const cv::Mat &t){
...
void MapManager::applyOptimizedMapPoint(int mappointId, const cv::Point3d &p){
...
void MapManager::updateMapPointDescriptor(MapPoint &mp) {
    ...
    if(descriptors[0].type() == CV_8U) {
        ...
        cv::Mat median = cv::Mat::zeros(1, bytes, CV_8U);
