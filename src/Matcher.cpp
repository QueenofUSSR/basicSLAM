...
void Matcher::match(const cv::Mat &desc1, const cv::Mat &desc2,
                    ...
                    int maxTotal, bool enableMutual, bool enableBucketing)
{
    ...
    struct Cand { cv::DMatch m; float x; float y; };
    ...
    std::vector<cv::DMatch> interimMatches;
    ...
    std::sort(interimMatches.begin(), interimMatches.end(), [](const cv::DMatch &a, const cv::DMatch &b){ return a.distance < b.distance; });
