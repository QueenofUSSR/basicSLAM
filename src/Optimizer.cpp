// Clean, single-definition Optimizer.cpp
...
void Optimizer::localBundleAdjustment(
    ...
    for (int idx : localKfIndices) {
        ...
        Eigen::Matrix3d R;
        cv::cv2eigen(kf.R_w, R);
        Eigen::Vector3d t;
        cv::cv2eigen(kf.t_w, t);
    ...
    for (size_t i = 0; i < mappoints.size(); ++i) {
        ...
        for (const auto &obs : mp.observations) {
            ...
            const cv::KeyPoint &kp = kf.kps[kpIdx];
...
bool Optimizer::optimizePose(
    ...
    cv::Mat rvec, tvec, inliersMask;
    bool success = cv::solvePnPRansac(objectPoints, imagePoints, K, cv::Mat(), rvec, tvec,
...
void Optimizer::globalBundleAdjustment(
    ...
    localBundleAdjustment(keyframes, mappoints, localKfIndices, fixedKfIndices, fx, fy, cx, cy, iterations);
...
double Optimizer::computeReprojectionError(
    ...
    cv::Mat Xw = (cv::Mat_<double>(3,1) << point3D.x, point3D.y, point3D.z);
    cv::Mat Xc = R.t() * (Xw - t);
    ...
    jacobianPose = cv::Mat::zeros(2,6,CV_64F);
    jacobianPoint = cv::Mat::zeros(2,3,CV_64F);
