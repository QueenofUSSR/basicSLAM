...
cv::vo::VisualOdometry::VisualOdometry(cv::Ptr<cv::Feature2D> feature, cv::Ptr<cv::DescriptorMatcher> matcher)
    : feature_(std::move(feature)), matcher_(std::move(matcher)) {
}

int cv::vo::VisualOdometry::run(const std::string &imageDir, double scale_m, const VisualOdometryOptions &options){
    ...
    // (previous-frame storage declared outside loop)
        ...
        if(!prevGray.empty() && !prevDesc.empty() && !desc.empty()){
            ...
            auto ratioKeep = [&](const std::vector<std::vector<cv::DMatch>>& knn, bool forward) {
                std::vector<cv::DMatch> filtered;
                for(size_t qi=0; qi<knn.size(); ++qi){
                    if(knn[qi].empty()) continue;
                    cv::DMatch best = knn[qi][0];
                    float ratio = 0.75f;
                    if(knn[qi].size() >= 2){
            ...
            std::vector<cv::DMatch> goodMatches = ratioKeep(knn12, true);
